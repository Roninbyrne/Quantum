...
from Quantum.utils.database import is_served_user
...
@Mukesh.on_message(filters.text, group=7)
async def bot_Send_msg(_, message):
    ...
    try:
       ...
       # if x==False:
        ...
        if message.chat.type==ChatType.SUPERGROUP and message.reply_to_message.from_user.id==Mukesh.id:
            ...
            if existing_group:
                ...
                output=api.chatgpt(message.text,mode="gf")
                
                await message.reply_text(text=output["results"],quote=True)
    except:
        pass
    try:
        if message.chat.type==ChatType.PRIVATE and message.reply_to_message.from_user.id ==Mukesh.id:
